...
### Quick Invocation Shortcut

**[COMMUNITY] The "Use Subagents" Trigger**

**Source**: Boris Cherny (Claude Code creator) team practices

**The Shortcut**: Append `"use subagents"` to any request where you want Claude to throw more compute at the problem.

```markdown
"Refactor this codebase to use the new API. Use subagents."

"Write tests for all edge cases. Use subagents."

"Analyze this 10,000 line file for security issues. Use subagents."
```

**What This Signals**:
1. Task is parallelizable or multi-phased
2. Heavy lifting required (keep main context clean)
3. User wants maximum compute thrown at problem
4. Results should cascade back as summaries

**Claude's Response**:
When Claude sees this phrase, it immediately considers:
- Can this be broken into parallel subtasks?
- Should I use `Task()` with specific agents?
- Which parts can run in isolated contexts?
- How do results merge back?

**Examples of Automatic Subagent Delegation**:

| Request With "Use Subagents" | Claude's Approach |
|:-----------------------------|:------------------|
| "Find all TODOs in codebase" | Spawns Explore agents per directory, aggregates results |
| "Migrate 50 files to new syntax" | Batches files into parallel Task() invocations |
| "Review this PR thoroughly" | Forks auditor skills for unbiased validation |

**Key**: Two words unlock massive parallelism. Use when you want Claude to parallelize automatically.
...
- **[03-hooks-specific.md](./03-hooks-specific.md)** - Event-driven security (hooks) and external capabilities (MCP)
...
**Next**: Implement 03-hooks-specific.md (Hooks + MCP components)
