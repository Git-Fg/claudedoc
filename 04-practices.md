...
> **Note**: This is a custom thecattoolkit_v3 pattern, NOT an official Claude Code command.
...
**[COMMUNITY] High-Level Delegation Patterns**

**Source**: Boris Cherny (Claude Code creator) team practices

For complex or ambiguous tasks, use high-level prompting that delegates autonomy to Claude rather than micromanaging implementation details.

**Pattern: Challenge-Based Review**

```markdown
"Grill me on these changes and don't make a PR until I pass your test."
```

Make Claude the reviewer. It will analyze your changes, ask probing questions, and refuse to proceed until you've adequately addressed concerns. This flips the dynamic—Claude holds you accountable rather than vice versa.

**Pattern: Proof of Correctness**

```markdown
"Prove to me this works."
"Diff behavior between main and this feature branch."
```

Don't ask Claude to implement verification—ask it to prove correctness. Claude will:
1. Identify what needs testing
2. Run comparative tests between branches
3. Present evidence of behavior changes
4. Flag any unexpected side effects

**Pattern: Elegant Redesign**

```markdown
"Knowing everything you know now, scrap this and implement the elegant solution."
```

When a fix feels mediocre or hacky, this prompt gives Claude permission to:
1. Discard the current approach entirely
2. Reconsider from first principles
3. Design a cleaner, more maintainable solution
4. Leverage insights gained during the initial attempt

**Pattern: Specification-First**

```markdown
"Write a detailed spec and reduce ambiguity before handing work off to implementation."
```
...
The more specific your spec, the better the output. Ambiguity in requirements produces ambiguity in results.

**Why High-Level Delegation Works**:

| Micromanagement | High-Level Delegation |
|:----------------|:----------------------|
| "Change line 5 to X" | "Fix the failing CI tests" |
| "Add a null check here" | "Handle edge cases properly" |
| "Use this specific library" | "Choose appropriate tooling for this task" |

**Key**: Claude often knows better than you do. Give it objectives and constraints, not step-by-step instructions.
...
**[CUSTOM] Quality Gate Workflow**

> **Note**: This is a custom thecattoolkit_v3 pattern. The `/quality:run-gates` command is a thecattoolkit_v3 implementation that automates this workflow.
...
**[CUSTOM] Session Handoff Workflow**
...
> **Note**: These are custom thecattoolkit_v3 commands, NOT official Claude Code commands.
...
**[COMMUNITY] Learning with Claude Code**

**Source**: Boris Cherny (Claude Code creator) team practices

Claude Code can be configured for educational output and learning workflows.
...
With this setting, Claude explains the *why* behind its changes, not just the *what*:
- "I'm using this pattern because..."
- "This approach handles the edge case where..."
- "The trade-off here is..."

**2. Visual Code Presentations**

Have Claude generate visual HTML presentations explaining unfamiliar code:

```markdown
"Create a visual HTML presentation explaining how the authentication flow works."
```

Claude will:
- Analyze the codebase
- Create slide-based HTML output
- Include diagrams and code snippets
- Structure it for presentation/education
...
```markdown
"Draw an ASCII diagram of the request handling pipeline."
"Show me the data flow between services in ASCII art."
```
...
**4. Spaced Repetition Learning Skill**

Build a custom skill for active learning:

```markdown
"Explain your understanding of how React hooks work."
"I'll ask follow-ups to fill gaps in your knowledge."
"Store the result for future reference."
```

The skill:
- Prompts you to explain concepts
- Asks probing follow-up questions
- Identifies knowledge gaps
- Stores Q&A for spaced repetition review
- Tracks understanding over time

**Key**: Learning is not passive consumption—it's active explanation with feedback.
...
**[COMMUNITY] High-Level Bug Fixing Delegation**

**Source**: Boris Cherny (Claude Code creator) team practices

Claude can fix most bugs autonomously when given the right context and high-level direction. Don't micromanage the fix—delegate the problem.

**Pattern: Slack-to-Fix**
...
**What happens**:
1. Claude reads the bug report from Slack thread
2. Analyzes error messages, stack traces, context
3. Searches codebase for relevant files
4. Identifies root cause
5. Implements fix
6. Zero context switching required

**Requirements**: Slack MCP server configured, bug thread contains error details

**Pattern: CI Self-Healing**

```markdown
"Go fix the failing CI tests."
```

Don't tell Claude how—just point it at the problem:
- Claude checks CI logs automatically
- Identifies which tests are failing
- Analyzes the failure reasons
- Makes necessary code changes
- Re-runs tests to verify

**Trust Level**: High. Claude has full context of the codebase and test suite.

**Pattern: Distributed System Troubleshooting**
...
| Approach | Your Role | Claude's Role |
|:---------|:----------|:--------------|
| **Micromanagement** | Specify exact changes | Execute blindly |
| **High-level delegation** | Define problem and success criteria | Analyze, decide, implement |

**Key**: Claude often spots patterns you miss. Give it logs, errors, and context—let it diagnose.
...
> **Note**: All commands listed here are custom thecattoolkit_v3 patterns, NOT official Claude Code commands.
