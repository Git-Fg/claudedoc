...
CLAUDE.md should improve continuously through a feedback loop:

**After Every Correction**:
```markdown
End every correction session with: "Update your CLAUDE.md so you don't make that mistake again."
```

Claude is remarkably good at writing rules for itself. When it makes a mistake:
1. Ask it to analyze the root cause
2. Have it propose a rule to prevent recurrence
3. Add the rule to CLAUDE.md immediately
4. The next session, Claude will follow the new rule automatically

**Ruthless Editing Cycle**:
- Keep iterating on CLAUDE.md over time
- Measure Claude's mistake rate on known problem areas
- Remove rules that don't reduce errors
- Add rules for every new failure mode encountered
- Aim for measurable improvement, not perfection

**Per-Project Notes Pattern**:
Some engineers have Claude maintain a `notes/` directory for every task/project, updated after every PR. They then point CLAUDE.md at these notes. This creates:
1. Session-specific learnings captured immediately
2. Project-contextual rules separate from global ones
