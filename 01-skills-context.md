...
For **event-driven automation** (Hooks), see [03-hooks-specific.md](03-hooks-specific.md).

For **external capabilities** (MCP servers), see [06-mcp.md](06-mcp.md).
...
### Skills Creation Heuristics

**[COMMUNITY] When to Create Skills**

**Source**: Boris Cherny (Claude Code creator) team practices

**The Rule**: *"If you do something more than once a day, turn it into a skill or command."*

This productivity heuristic from the Claude Code team identifies high-leverage automation opportunities:

| Frequency | Action |
|:----------|:-------|
| **Once per day** | Create a skill |
| **Multiple times per day** | Create a command (faster invocation) |
| **Once per week** | Document the pattern, decide later |
| **Rarely** | Keep as manual process |

**High-Value Skill Ideas from the Team**:
...
Runs before session end:
- Scans for duplicated patterns
- Identifies dead code
- Suggests consolidations
- Creates a "tech debt report"

**2. Context Sync Aggregator**
```markdown
Set up a slash command that syncs 7 days of Slack, GDrive, Asana, and GitHub into one context dump.
```

Before starting a task:
- Pulls recent relevant activity
- Summarizes context from all sources
- Creates a "session briefing" document
- Reduces context-switching overhead

**3. Analytics Engineer Agent**
```markdown
Build analytics-engineer-style agents that write dbt models, review code, and test changes in dev.
```

Specialized agent skills:
- Writes dbt models from specifications
- Reviews SQL for optimization
...
1. **Observe** patterns in daily work
2. **Measure** repetition frequency
3. **Automate** when threshold crossed
