...
|:--------|:-----|:----------|
| Quick Reference Tables | ~15 | frontmatter, agents, context, hooks, MCP |
...
| MCP Reference | → [06-mcp.md](06-mcp.md) | servers, tools, transport types |
...
| `agent` | string | No (defaults to general-purpose) | Which agent type (Explore/Plan/general-purpose) |
...
> **Note**: These are project-specific terms for describing context behavior. The official documentation describes these behaviors but does not use this specific terminology.
...
|:------|:------|:---------|
| `SessionStart` | Session init/resume | Load discipline context |
| `UserPromptSubmit` | User submits prompt | Validate/transform prompts |
| `PreToolUse` | Before any tool | Security, validation, middleware |
| `PermissionRequest` | Permission dialog | Auto-allow/deny permissions |
| `PostToolUse` | After tool | Logging, notifications |
| `PostToolUseFailure` | Tool fails | Error handling, retries |
| `Notification` | Notification sent | Respond to alerts |
| `SubagentStart` | Subagent spawns | Setup subagent context |
| `SubagentStop` | Subagent ends | Cleanup, archive |
| `Stop` | Claude stops | Final cleanup |
| `PreCompact` | Before compaction | Preserve context |
| `SessionEnd` | Session ends | Session archive, metrics |

**Hook Types** (3 types):

| Type | Purpose | Returns |
|:-----|:--------|:--------|
| `command` | Execute shell script | Exit code + JSON stdout |
| `prompt` | Single-turn LLM evaluation | JSON decision {ok, reason} |
| `agent` | Multi-turn subagent | JSON decision {ok, reason} |
